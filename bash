...
packages(){
	...
	if ! command -v ffmpeg &>/dev/null; then
		...
		if sudo apt install ffmpeg -y &>/dev/null;then
			...
			pprint "FAIL.\n\n" "cred"
			pprint "You need to install ffmpeg manually in order to deploy venomXddos, exiting...\n" "cblue"
			exit 1
		fi
	fi
...
clear
pprint "Welcome to VENOMxDDOS Setup Installer\n\n"
...
node
installation
pprint "\n\n\n\n\VENomxDDOS Installation Completed !" "cgreen"
sleep 1
clear

pprint "\nEnter Your Values Below\n\n\n"
pprint "BOT TOKEN: "; color_reset; read BOT_TOKEN
pprint "GROUP ID: "; color_reset; read GROUP_ID
...
pprint "\n\n\nThanks for using VENOMxDDOS installer, your vars have been saved successfully ! \nIf you wanna add more variables add them in your env by : vi .env"
